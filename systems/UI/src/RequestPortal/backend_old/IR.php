...
if(isset($_POST['key'])){
   ...
   FROM request_portal 
   WHERE  (Request_for LIKE '%IT%' OR Request_for LIKE '%HALL%') AND NOT(Status='Completed' OR Status='Cancelled') 
   AND(
      item_id LIKE '%$key%' OR
 ...
 SELECT item_id,Req_Name,Description,Location,Due_date,Due_time,Handler,Status 
 FROM request_portal 
 WHERE Request_for LIKE  (Request_for LIKE '%IT%' OR Request_for LIKE '%HALL%') AND NOT(Status='Completed') ORDER BY item_id DESC ";
