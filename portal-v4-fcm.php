<?php
    ...
    if(isset($_SESSION['login_user'])&&isset($_SESSION['login_name'])){
    ...
    <body id='bd' class="vertical" onload="triggerOnLoad()">      
        ...
        <section id="tile-wrapper" class="clearfix section">
            <section id='systems' class="clearfix section">
                ...
                -->
                <?php
                    $sql_comp = "SELECT * FROM portal_components WHERE status='enable'";
                    $result_comp = $db->query($sql_comp);
                    $outputlist = '';
                    if(mysqli_num_rows($result_comp)>0){
                        while($row_comp = mysqli_fetch_array($result_comp)){
                            ...
                            $sname = $row_comp['name'];
                            $href = $row_comp['href'];
                            $color = $row_comp['color'];
                            $icon = $row_comp['icon'];
                            $status = $row_comp['status'];
                            $outputlist.="
                            <div class='tile $color w2 h1 title-verticalcenter icon-featurefade isotope-item'>
